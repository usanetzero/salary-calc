...
import {
  MapPin, Home, DollarSign, Zap, Car, Utensils, Activity,
  TrendingUp, ChevronRight, BarChart2, Users, Percent,
  AlertCircle
} from "lucide-react";
...
import { formatCurrency, formatNumber, costIndexLabel, costIndexBg, costIndexColor } from "@/lib/utils";

const NEARBY_CITY_SLUGS: Record<string, string[]> = {
  "new-york-ny": ["boston-ma", "baltimore-md", "washington-dc", "pittsburgh-pa"],
  "los-angeles-ca": ["san-diego-ca", "san-francisco-ca", "las-vegas-nv", "phoenix-az"],
  "chicago-il": ["milwaukee-wi", "indianapolis-in", "detroit-mi", "st-louis-mo"],
  ...
  "san-jose-ca": ["san-francisco-ca", "sacramento-ca", "los-angeles-ca", "seattle-wa"],
  "austin-tx": ["dallas-tx", "houston-tx", "san-antonio-tx", "nashville-tn"],
  "seattle-wa": ["portland-or", "denver-co", "san-francisco-ca", "los-angeles-ca"],
  "denver-co": ["colorado-springs-co", "salt-lake-city-ut", "albuquerque-nm", "austin-tx"],
...
function MetricBar({ value, max = 200, label }: { value: number; max?: number; label?: string }) {
  const pct = Math.min((value / max) * 100, 100);
  const color = value < 90 ? "#10b981" : value < 110 ? "#f59e0b" : value < 140 ? "#f97316" : "#ef4444";
  return (
...
export default function CityPageClient() {
  ...
  const nearbyCities = allCities?.filter((c) => nearbySlugs.includes(c.slug) || c.stateCode === city?.stateCode).slice(0, 4) || [];
  ...
  if (isError || !city) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-muted-foreground mb-6">We couldn&apos;t find data for this city.</p>
  ...
  const adjustedSalary = compareSlug && allCities
    ? salary * ((allCities.find((c) => c.slug === compareSlug)?.costIndex || 100) / city.costIndex)
    : null;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
        ...
        <Link href="/cheapest-cities"><span className="cursor-pointer hover-elevate rounded-sm">Cities</span></Link>
        <ChevronRight className="w-3 h-3" />
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground">{city.state} · Population: {formatNumber(city.population)}</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link href={`/compare/${city.slug}/vs/${nearbyCities[0]?.slug || "dallas-tx"}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Cost Index", value: city.costIndex.toFixed(1), icon: BarChart2, sub: "100 = US avg", color: costIndexColor(city.costIndex) },
          { label: "Median Income", value: formatCurrency(city.medianIncome, { compact: true }), icon: DollarSign, sub: "Annual household" },
          { label: "Median Rent", value: `${formatCurrency(city.medianRent)}/mo`, icon: Home, sub: "2-bedroom unit" },
          { label: "Rent Burden", value: `${rentBurden.toFixed(0)}%`, icon: Percent, sub: "Income to rent ratio" },
        ].map((metric) => (
          <Card key={metric.label}>
            <CardContent className="p-4">
              ...
              <div className="text-xs text-muted-foreground mt-0.5">{metric.sub}</div>
            </CardContent>
          </Card>
        ))}
      </div>
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardHeader className="pb-4">
              ...
              <p className="text-xs text-muted-foreground">Indexed to national average (100)</p>
            </CardHeader>
            <CardContent>
              <Tabs defaultValue="bars">
                ...
                <TabsContent value="bars">
                  <div className="space-y-3">
                    ...
                    <MetricBar label="Transportation" value={city.transportationIndex} />
                    <MetricBar label="Healthcare" value={city.healthcareIndex} />
                  </div>
                </TabsContent>
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-3">
              {[
                { label: "Median Home Value", value: formatCurrency(city.medianHomeValue, { compact: true }), icon: Home },
                { label: "State Income Tax", value: city.taxRate === 0 ? "None" : `${city.taxRate}%`, icon: Percent },
                { label: "Unemployment Rate", value: `${city.unemploymentRate}%`, icon: Users },
                { label: "Est. Net Income", value: formatCurrency(netIncome, { compact: true }) + "/yr", icon: DollarSign },
              ].map((item) => (
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-center py-2">
                <div className={`text-4xl font-bold mb-1 ${costIndexColor(city.costIndex)}`}>
                  {Math.round(100 - ((city.costIndex - 100) * 0.5)).toString().padStart(2, "0")}
                </div>
                <div className="text-sm text-muted-foreground">out of 100</div>
                <div className="text-xs text-muted-foreground mt-2">
                  {city.costIndex < 100 ? "Below national average cost" : "Above national average cost"}
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <Card className="mb-8">
        <CardHeader className="pb-4">
          ...
          <p className="text-sm text-muted-foreground">
            See what salary you&apos;d need in another city to match your purchasing power in {city.name}.
          </p>
        </CardHeader>
        <CardContent className="space-y-5">
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium">Salary in {city.name}</label>
              <span className="text-xl font-bold text-primary">{formatCurrency(salary)}</span>
            </div>
            <Slider
              value={[salary]}
          ...
          <div>
            <label className="text-sm font-medium mb-2 block">Compare to City</label>
          ...
          {adjustedSalary !== null && compareCityData && (
            <div className="rounded-md bg-accent p-4 grid sm:grid-cols-2 gap-4">
              <div>
                <div className="text-xs text-muted-foreground mb-1">{city.name}</div>
                <div className="text-xl font-bold">{formatCurrency(salary)}</div>
                <div className="text-xs text-muted-foreground">
                  Cost Index: {city.costIndex.toFixed(1)}
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground mb-1">{compareCityData.name}</div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {nearbyCities.map((nearCity) => (
              <Link key={nearCity.slug} href={`/compare/${city.slug}/vs/${nearCity.slug}`}>
                <Card className="cursor-pointer hover-elevate">
                  <CardContent className="p-4 text-center">
                    <div className="font-medium text-sm mb-1">{nearCity.name}</div>
                    <div className={`text-lg font-bold ${costIndexColor(nearCity.costIndex)}`}>
                      {nearCity.costIndex.toFixed(0)}
                    </div>
                    <div className="text-xs text-muted-foreground">Cost Index</div>
                    <Badge className={`mt-2 text-xs ${costIndexBg(nearCity.costIndex)}`}>
