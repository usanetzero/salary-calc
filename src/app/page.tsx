...
import {
  MapPin, TrendingUp, Calculator, ArrowRight, DollarSign,
  Home as HomeIcon, Utensils, Zap, Car, Activity, ChevronRight,
...
const TRENDING_COMPARISONS = [
  { slugA: "austin-tx", slugB: "dallas-tx", cityA: "Austin", cityB: "Dallas", stateA: "TX", stateB: "TX" },
  { slugA: "new-york-ny", slugB: "los-angeles-ca", cityA: "New York", cityB: "Los Angeles", stateA: "NY", stateB: "CA" },
  { slugA: "seattle-wa", slugB: "san-francisco-ca", cityA: "Seattle", cityB: "San Francisco", stateA: "WA", stateB: "CA" },
  { slugA: "miami-fl", slugB: "nashville-tn", cityA: "Miami", cityB: "Nashville", stateA: "FL", stateB: "TN" },
  { slugA: "denver-co", slugB: "austin-tx", cityA: "Denver", cityB: "Austin", stateA: "CO", stateB: "TX" },
  { slugA: "chicago-il", slugB: "dallas-tx", cityA: "Chicago", cityB: "Dallas", stateA: "IL", stateB: "TX" },
];
        ...
        <CardContent className="p-5">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className={`font-semibold ${costIndexColor(city.costIndex)}`}>
            ...
            <div className="flex items-center justify-between">
              <div className="w-full bg-muted rounded-full h-1.5">
                <div
                  ...
                  style={{ width: `${Math.min((city.costIndex / 200) * 100, 100)}%` }}
                />
              </div>
              <span className="text-xs text-muted-foreground ml-2 shrink-0">vs avg</span>
            </div>
          </div>
        </CardContent>
      </Card>
    ...
    <Card className="shadow-lg">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-sm font-medium mb-1.5 block">From City</label>
        ...
        {adjustedSalary !== null && diff !== null && fromCityData && toCityData && (
          <div className="rounded-md bg-accent p-4 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Equivalent Salary in {toCityData.name}</span>
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className={diff > 0 ? "text-green-600 dark:text-green-400 font-medium" : "text-red-600 dark:text-red-400 font-medium"}>
                {diff > 0 ? "+" : ""}{formatCurrency(diff)}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Cost Index: {fromCityData.name}</span>
              <span className="font-medium">{fromCityData.costIndex.toFixed(1)}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Cost Index: {toCityData.name}</span>
              <span className="font-medium">{toCityData.costIndex.toFixed(1)}</span>
            </div>
          </div>
        )}
    ...
    <div className="min-h-screen">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          <div className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-6">
              ...
              <span className="block text-white/80">Calculator for US Cities</span>
            </h1>
            <p className="text-lg md:text-xl text-white/85 mb-8 leading-relaxed max-w-2xl">
              Compare cost of living index across 47 major US cities. Calculate salary equivalents,
              compare housing costs, state taxes, rent prices, and find the cheapest cities to live in America — powered by Census, BEA, HUD, and BLS data.
            </p>
            <div className="flex flex-wrap gap-3">
              ...
              <Link href="/cheapest-cities">
                <Button size="lg" variant="outline" className="gap-2 text-white border-white/40 bg-white/10">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              { label: "National Avg Income", value: "$74,580", icon: DollarSign },
            ].map((stat) => (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-16">
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-start">
            <div>
              ...
              <p className="text-muted-foreground leading-relaxed mb-6">
                A $100,000 salary in New York City has the purchasing power of just $59,000 in
                terms of national average. Our calculator adjusts for the full cost of living
                difference between any two cities.
              </p>
              <div className="space-y-3">
                {[
                  { icon: HomeIcon, text: "Housing costs including rent and mortgage payments" },
                  { icon: Utensils, text: "Grocery and dining out price differences" },
                  { icon: Zap, text: "Utilities — electricity, gas, water, internet" },
                  { icon: Car, text: "Transportation costs including gas and transit" },
                  { icon: Activity, text: "Healthcare costs by metropolitan area" },
                ].map((item) => (
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {TRENDING_COMPARISONS.map((comp) => {
              ...
              return (
                <Link key={`${comp.slugA}-${comp.slugB}`} href={`/compare/${comp.slugA}/vs/${comp.slugB}`}>
                  <Card className="cursor-pointer hover-elevate">
                    <CardContent className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="flex-1 text-center">
                          <div className="font-semibold text-sm">{comp.cityA}</div>
                          <div className="text-xs text-muted-foreground">{comp.stateA}</div>
                          {cityAData && (
                            <div className={`text-sm font-bold mt-1 ${costIndexColor(cityAData.costIndex)}`}>
                              {cityAData.costIndex.toFixed(0)}
                            </div>
                          )}
                        </div>
                        <div className="text-muted-foreground font-medium text-sm px-2">vs</div>
                        <div className="flex-1 text-center">
                          <div className="font-semibold text-sm">{comp.cityB}</div>
                          <div className="text-xs text-muted-foreground">{comp.stateB}</div>
                          {cityBData && (
                            <div className={`text-sm font-bold mt-1 ${costIndexColor(cityBData.costIndex)}`}>
        ...
        <FAQSection
          ...
          items={[
            {
              question: "What is a cost of living index and how is it calculated?",
              answer: "A cost of living index measures the relative price levels across different geographic areas. CostWise uses the Bureau of Economic Analysis Regional Price Parities (RPP), which compare price levels in each metro area to the national average of 100. An index of 110 means prices are 10% above the national average, while 90 means 10% below.",
            },
            {
              question: "How much salary do I need to earn to maintain my lifestyle in a new city?",
              answer: "To maintain the same standard of living, your salary should adjust proportionally to the cost of living difference. For example, if you earn $100,000 in a city with an index of 100 and move to a city with an index of 120, you'd need roughly $120,000. Our salary calculator accounts for housing costs, taxes, groceries, utilities, and transportation to give you a precise figure.",
            },
            {
              question: "What are the cheapest major cities to live in the United States?",
              answer: "Based on our data, the most affordable major US cities include Oklahoma City, Memphis, Wichita, El Paso, and Indianapolis. These cities have cost of living indices 10-15% below the national average, with particularly low housing costs. Several also benefit from no state income tax.",
            },
            {
              question: "How does state income tax affect my take-home pay when relocating?",
              answer: "State income tax can significantly impact your take-home pay. States like Texas, Florida, Tennessee, Nevada, and Washington have no state income tax, while California charges up to 13.3% and New York up to 10.9%. Moving from California to Texas on a $100,000 salary could save you over $10,000 per year in state taxes alone.",
            },
            {
              question: "Where does CostWise get its data?",
              answer: "CostWise aggregates data from four official US government sources: the Bureau of Economic Analysis (Regional Price Parities), US Census Bureau (American Community Survey 5-year estimates for income, rent, and home values), HUD Fair Market Rents, and the Bureau of Labor Statistics (unemployment rates and Consumer Price Index sub-components for groceries, utilities, and transportation).",
            },
            {
              question: "How often is the cost of living data updated?",
              answer: "Our data is updated annually when new government figures are released. Census ACS data is typically released each December, BEA price parities in late summer, HUD Fair Market Rents in fall, and BLS unemployment data monthly with annual averages published in January.",
            },
          ]}
        />
        ...
        <section className="text-center py-8">
          ...
          <p className="text-muted-foreground mb-6 max-w-lg mx-auto">
            Use our full salary calculator for a complete breakdown including tax analysis,
            rent ratios, and lifestyle comfort scores.
          </p>
